...
const FeedPost = (props:{id:string,image:string,username:string,date:string , likes:number,comment:string,avatarurl:string}) => {
    ...
    const handleLike = async () => {
        try {
            const response = await axios.post(`/api/like?p=${props.id}`);
            if (response.status === 200) {
                setLike(!like);
                setLikeCount(like ? likeCount - 1 : likeCount + 1);
                queryClient.invalidateQueries(); // Optionally, to refetch the like status
            } else {
