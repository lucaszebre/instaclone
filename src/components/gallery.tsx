...
import axios from 'axios';
    ...
    const Gallery = (props:{photos?: GalleryType[]} ) => {
        ...
        {props.photos &&props.photos.map((photo, index) => (
            ...
            <PhotoCard filekey={photo.filekey?photo.filekey:""} id={photo.id} userId={photo.userId} avatar={photo.user.profilePictureUrl || ''} username={photo.user.username} key={index} image={photo.imageUrl} like={photo.likes ? photo.likes.length.toString() : "0"} comment={photo.comments ? photo.comments :[]} imgdescription={'dd'} likes={photo.likes?photo.likes:[]} />
        ))}
        </div>
    );
    }
