...
export function Sidebar() {
  ...
  return (
<div className={`${side!=='home'?'w-[96px]':''} absolute z-10 bg-white bottom-0 w-full md:fixed h-[48px] flex md:flex-col flex-row  xl:max-w-[250px] md:max-w-[96px] md:h-screen md:border-black-400 md:border-r-2`}>
    <div className={`${side!=='home'?'w-[96px]':''} flex md:flex-col flex-row md:py-4 h-full xl:max-w-[250px] md:max-w-[96px]`} >
        <div className="md:px-3   md:py-2 flex md:flex-col flex-row  justify-between h-full max-md:w-full">
            <div  className={`${side!=='home'?'hidden':'xl:flex hidden'} mb-[20px] `}>
           <Button  variant="link" className={` w-full justify-center md:justify-start gap-5`}>
            ...
            <div className={`${side!=='home'?'flex':'xl:hidden'} mb-[20px]  md:flex justify-center hidden  `}>
            ...
            <div className="md:space-y-1 md:flex md:flex-col flex-row md:w-full w-screen  grid grid-cols-5 grid-rows-1 md:justify-center">
                <Button onClick={()=>setSide('home')} variant="ghost" className="h-full md:h-[50px]   justify-center md:justify-start gap-5 ">
                ...
                {side==='home' ?<p className={`${side=='home'?'font-bold':''} xl:flex  hidden`}>Home</p>:null}
                </Button>
                <Search>
                ...
                <AddFile>
                  <Button  variant="ghost" className="w-full h-full md:h-[50px] justify-center md:justify-start gap-5">
                      ...
                      {side !== 'search' ? <p className={` xl:flex  hidden`}>Create</p> :null}
                  </Button>
                </AddFile>
                    

                
                    <Button onClick={()=> setSide('messages')} variant="ghost" className="h-full md:h-[50px]  justify-center md:justify-start gap-5">
                    ...
                    {side!=='messages' ? <p className={`${side=='messages'?'font-bold':''} xl:flex  hidden`}>Messages</p> :null}
                    </Button>

              
                
                <Button  variant="ghost" className="w-full  justify-center md:justify-start gap-5 md:hidden flex  xl:hidden">
                ...
                <Button onClick={()=> setSide('profile')} variant="ghost" className=" h-[50px] justify-center md:justify-start gap-5">
                <Avatar className="hover:w-[26px] hover:h-[26px] w-[24px] h-[24px]" >
                    <AvatarImage src="https://github.com/shadcn.png" />
                    <AvatarFallback>CN</AvatarFallback>
                </Avatar>
                {side!=='profile' ? <p className="xl:flex  hidden">Profil</p> :null}
                </Button>
                </div>
            
            <div className="md:flex hidden">
                <Plus>
                  <Button onClick={()=> setSide('plus')} variant="ghost" className="w-full h-full md:h-[50px] justify-start gap-5">
                                ...
                                {side!=='plus' ? <p className={`${side=='plus'?'font-bold':''}  xl:flex  hidden`}>More</p> :null}
                  </Button>
                </Plus>
