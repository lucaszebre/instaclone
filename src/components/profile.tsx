...
const Profile = (props:{profile:Usered}) => {
    ...
    const {
      ...
      const savePost = useQuery({
        ...
        const [isFollow,setIsFollow]= useState(data?.following?data?.following.some(following => following.followingId === props.profile.id):false)
        ...
        useEffect(()=>{
        setIsFollow(data?.following?data?.following.some(following => following.followingId === props.profile.id):false)
        },[data?.following?data?.following.some(following => following.followingId === props.profile.id):false])
    ...
    return (
        ...
        <div className='  max-w-[975px] pt-[60px] md:pt-[0px] h-full w-screen md:w-full'>
            <div className=' w-full justify-between flex flex-row h-[190px]'>
                ...
                <div className=' h-full flex flex-col justify-center p-8 md:p-0 gap-4 content-center w-full'>
        ...
        <Button
            ...
            variant={'ghost'}          
            className="text-sm font-medium flex-row flex gap-2 items-center transition-colors hover:text-primary"
        >
        ...
        <Button onClick={()=>{setSave(true)}}
            variant={'ghost'}
            className="text-sm font-medium flex-row flex gap-2 items-center text-muted-foreground transition-colors hover:text-primary"
        >   <svg aria-label="" className="x1lliihq x1n2onr6 x1roi4f4" fill="currentColor" height="20" role="img" viewBox="0 0 24 24" width="20"><title></title><polygon fill="none" points="20 21 12 13.44 4 21 4 3 20 3 20 21" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"></polygon></svg>
            SAVED
        </Button>
